...
class GUI:
    ...
    def draw_move_time(self, window, game, color, position_multiplier):
        font_size = 25
        if color == "white":
            self.draw_text(window, f'{utils.format_time(game.white_move_clock)}', (COLS-1)*SQUARE_SIZE-20, BOARD_FRAME_WIDTH+15 + position_multiplier*SQUARE_SIZE, font_size, WHITE_COLOR, self.font25)
        else: 
            self.draw_text(window, f'{utils.format_time(game.black_move_clock)}', (COLS-1)*SQUARE_SIZE-20, (BOARD_FRAME_WIDTH/2)-5 + position_multiplier*SQUARE_SIZE, font_size, WHITE_COLOR, self.font25)

    def draw_clock(self, window, game):
        self.draw_text(window, f'{utils.format_time(game.white_clock)}', COLS*SQUARE_SIZE, BOARD_FRAME_WIDTH+10 + 9*SQUARE_SIZE, 35, WHITE_COLOR, self.font35)
        self.draw_text(window, f'{utils.format_time(game.black_clock)}', COLS*SQUARE_SIZE, (BOARD_FRAME_WIDTH/2)-10 + 0*SQUARE_SIZE, 35, WHITE_COLOR, self.font35)


class PlayerCaptureStats(GUI):
    def __init__(self, color="white"):
        self.color = color
        self.piece_mini_images = utils.load_piece_images(width=50, height=50)
        if self.color == "white":
...
class PlayerSettings(GUI):
    def __init__(self, color):
        ...
        self.width = 310
        self.height = 115
        button_labels = ["     Player", "       AI", "   Easy", "Medium", "  Hard"]
...
class BoardGUI:
    def __init__(self):
        self.piece_images = utils.load_piece_images(width=SQUARE_SIZE, height=SQUARE_SIZE)
